...
import { heartbeatService, polymarketCopyService, reconcilePersistedRuntimeServicesOnStartup, routineService } from "./services/index.js";
...
export async function startServer(): Promise<StartedServer> {
  ...
  if (config.heartbeatSchedulerEnabled) {
    ...
    const polymarketCopy = polymarketCopyService(db as any);
    ...
    setInterval(() => {
      ...
      void polymarketCopy
        .tickAllCompanies(new Date())
        .catch((err) => {
          logger.error({ err }, "polymarket scheduler tick failed");
        });
